...
export default function TeacherDashboard() {
  ...
  const expirationRef = useRef(null);   // will hold the expiration timestamp
  const timeoutRef   = useRef(null);   // will hold the logout timeout ID
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(user => {
      ...
      // Must have teacherInfo in localStorage
      const info = JSON.parse(localStorage.getItem('teacherInfo'));
      if (!info) {
        // Data missing → treat as expired
      ...
      // Compute expiration timestamp (loginTime + 1hr)
      expirationRef.current = loginTime + 60*60*1000;

      // Schedule single logout at expiration
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const mins = Math.max(
        0,
        Math.ceil((expirationRef.current - now) / (60*1000))
      );
      setTimeLeft(mins);
    };
    tick(); // initial
  ...
  // --- 6) HELPERS FOR CREATING AN EXAM ---
  const addQuestion = () => {
    setQuestions(qs => [
      ...qs,
        ...
        {exams.length === 0
          ...
          : (
            <ul className="list-disc pl-6">
              {exams.map(e => (
                <li key={e.id}>{e.title} ({e.grade})</li>
              ))}
            </ul>
          )}
      </div>
